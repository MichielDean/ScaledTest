...
describe('Page Accessibility Tests', () => {
  ...
  beforeEach(() => {
    ...
    adminUsersPage = new AdminUsersPage(playwright.page);
  });
  ...
  describe('Public Pages', () => {
    ...
    test('Unauthorized page should be accessible', async () => {
      const basePage = new BasePage(playwright.page);
      await basePage.goto('/unauthorized');
      await waitForPageLoad(playwright.page, 1000);
  ...
  describe('Authenticated Pages', () => {
    ...
    authenticatedPages.forEach(({ name, pageObject }) => {
      test(`${name} page should be accessible`, async () => {
        const page = pageObject();
        await page.goto();
        await waitForPageLoad(playwright.page, 1500); // Longer wait for authenticated pages
    ...
    test('Test Results Dashboard should be accessible', async () => {
      ...
      await testResultsDashboardPage.waitForPageToLoad();
    ...
    test('Admin pages should be accessible', async () => {
      // Login as OWNER user who has admin privileges
