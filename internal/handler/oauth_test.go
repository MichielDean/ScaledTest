...
func newTestOAuthHandler(oauthCfgs *auth.OAuthConfigs) *OAuthHandler {
	...
	return &OAuthHandler{JWT: jwt, DB: nil, OAuth: oauthCfgs, Secure: false}
}
...
func TestGitHubCallback_NoDB(t *testing.T) {
	cfg := &auth.OAuthConfigs{
		GitHub: &oauth2.Config{
			ClientID:     "test-id",
			ClientSecret: "test-secret",
	...
	h := newTestOAuthHandler(cfg) // DB is nil
	...
	if w.Code != http.StatusServiceUnavailable {
		t.Errorf("GitHubCallback no DB: got %d, want %d", w.Code, http.StatusServiceUnavailable)
	}
}
...
func TestGitHubCallback_MissingState(t *testing.T) {
	...
	// Need a non-nil DB to get past the DB check
	h := &OAuthHandler{
		JWT:   auth.NewJWTManager(testSecret, 15*time.Minute, 7*24*time.Hour),
		DB:    nil, // We'll test the state check, but DB nil check comes first
		OAuth: cfg,
	}

	// With nil DB, we'll get 503 before state check. That's OK for this path.
	...
	// Should get 503 (no DB) since DB check happens before state check
	if w.Code != http.StatusServiceUnavailable {
...
func TestGitHubCallback_ProviderError(t *testing.T) {
	...
	// DB nil → 503 before provider error check. This tests the not-configured path.
	h := newTestOAuthHandler(cfg)
