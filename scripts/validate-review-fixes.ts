...
async function validateResolvedComments() {
  ...
  try {
    ...
    if (adminUsersContent.includes('pool.query') && !adminUsersContent.includes('const paginatedUsers: Array<> = [];')) {
    ...
    if (userRolesContent.includes('session?.user?.id') && !userRolesContent.includes("req.headers['user-id']")) {
