...
export class UserManagementPage extends BasePage {
  ...
  async expectUserListed(): Promise<void> {
    ...
    testLogger.info(
      `Main content text: ${mainContentText?.substring(0, 200)}...`,
    );
    ...
    if (skeletonVisible) {
      ...
      try {
        await this.page.waitForSelector(".space-y-4", {
          state: "hidden",
          timeout: 10000,
        });
    ...
    testLogger.info(
      `Table body exists: ${tableBodyExists}, visible: ${tableBodyVisible}`,
    );
    ...
    try {
      ...
      testLogger.info(
        `Direct API call result: ${JSON.stringify(response, null, 2)}`,
      );
    } catch (error) {
  ...
  async expectAccessDenied() {
    // With the protected admin pages, authenticated non-admin users are redirected to unauthorized page
    await expect(this.page).toHaveURL(/\/(unauthorized|login)/, { timeout: 5000 });
  }

  /**
   * Check if access denied message is shown when accessing without proper permissions
   * In the protected admin pages, authenticated non-admin users are redirected to unauthorized page
   */
  async expectUnauthorizedPage() {
    // With the protected admin pages, authenticated non-admin users are redirected to unauthorized page
    await expect(this.page).toHaveURL(/\/(unauthorized|login)/, { timeout: 10000 });
  }
}
