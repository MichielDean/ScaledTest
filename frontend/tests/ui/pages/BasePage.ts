...
export class BasePage {
  ...
  constructor(page: Page) {
    this.page = page;
    // Use baseURL from Playwright config (supports K8s and local modes)
    this.baseUrl = process.env.BASE_URL || "http://localhost:5173";
  }

  /**
   * Navigate to a specific URL path
   * Uses Playwright's configured baseURL (supports K8s and local modes)
   */
  async goto(path: string) {
    // Use relative path - Playwright will prepend baseURL from config
  ...
  async waitForPageLoad(timeout = 1500) {
    ...
    await this.page.waitForSelector("#main-content, main, body", {
      state: "visible",
    });
