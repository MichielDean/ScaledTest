...
export class AdminUsersPage extends BasePage {
  ...
  async expectPageLoaded(): Promise<void> {
    ...
    if (!currentUrl.includes("/admin/users")) {
      throw new Error(
        `Expected to be on admin/users page, but current URL is: ${currentUrl}`,
      );
    }
    ...
    if (isLoginPage) {
      throw new Error(
        "Redirected to login page - user may not have proper admin permissions",
      );
    }
    ...
    if (isUnauthorizedPage) {
      throw new Error(
        "Redirected to unauthorized page - user may not have owner role",
      );
    }
  ...
  async isUsersTableVisible(): Promise<boolean> {
    try {
      await this.page.waitForSelector("#users-table", {
        state: "visible",
        timeout: 5000,
      });
      return true;
