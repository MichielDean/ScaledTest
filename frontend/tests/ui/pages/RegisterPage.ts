...
export class RegisterPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.registerButton = page.locator("#signUpButton");
  ...
  async register(
    ...
    if (name) {
      await this.fillWithRetry(this.nameInput, name, "name");
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (attempts >= maxAttempts) {
          throw new Error(
            `Failed to fill ${fieldName} after ${maxAttempts} attempts: ${error}`,
          );
        }
